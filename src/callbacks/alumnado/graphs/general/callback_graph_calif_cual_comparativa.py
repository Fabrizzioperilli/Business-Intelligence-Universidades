...
def update_graph_alumnado(curso_academico, asignaturas_matriculadas, alumno_id, titulacion):
    """
    Actualiza el gráfico de calificaciones cualitativas general por curso académico
    ...
    fig.update_layout(
        title={
            "text": "Calificaciones cualitativas general por curso académico",
            "x": 0.5,
        },
        barmode="stack",
    ...
    general_data = calif_cualitativa_comparativa(
        curso_academico, asignaturas_matriculadas, titulacion, data_universidad[0][0]
    )
    student_data = calif_cualitativa_alumno_asignaturas(
    ...
    general_df = pd.DataFrame(
        general_data, columns=["Asignatura", "Calificacion", "Numero"]
    )
    student_df = pd.DataFrame(
        student_data, columns=["Asignatura", "Calificacion", "Numero"]
    )
    ...
    general_pivot = general_df.pivot_table(
        index="Asignatura", columns="Calificacion", values="Numero", fill_value=0
    )
