...
cod_plan_dict = {
    titulacion: fake.bothify(text="PLAN###??") for titulacion in asignaturas_dict.keys()
}
...
def generate_matricula(alumnos_df):
    ...
    for i, row in tqdm(alumnos_df.iterrows(), total=n, desc="matrícula"):
        ...
        for j in range(num_matriculas[i]):
            curso_aca = random.choice(
                [ca for ca in cursos_academicos if ca not in cursos_aca_usados]
            )
...
def generate_asignaturas_matriculadas(matricula_df, cod_asignaturas):
    asignaturas_data = []
    for i, row in tqdm(
        matricula_df.iterrows(),
        total=len(matricula_df),
        desc="asignaturas_matriculadas",
    ):
        num_asignaturas = min(
            random.randint(1, 20), len(asignaturas_dict[row["titulacion"]])
        )
        asignaturas = random.sample(
            asignaturas_dict[row["titulacion"]], num_asignaturas
        )
        for asignatura in asignaturas:
...
def generate_lineas_actas(asignaturas_matriculadas_df):
    actas_data = []
    for i, row in tqdm(
        asignaturas_matriculadas_df.iterrows(),
        total=len(asignaturas_matriculadas_df),
        desc="líneas_actas",
    ):
...
def generate_docentes(n, universidades, cod_asignaturas, asignaturas_dict):
    ...
    for i in tqdm(range(n), desc="docentes"):
        ...
        asignaturas_seleccionadas = random.sample(
            list(cod_asignaturas.items()), num_asignaturas
        )
...
def generate_ebau_prueba(n, alumnos_ids, universidades):
    assert n == len(
        alumnos_ids
    ), "El número de registros debe ser igual al número de alumnos"
...
def generate_egresados(alumnos_df, matricula_df, lineas_actas_df):
    aprobados = (
        lineas_actas_df[lineas_actas_df["calif_numerica"] >= 5].groupby("id").size()
    )
    ...
    egresados_data = []
    for i, id_alumno in tqdm(
        enumerate(egresados_ids), total=len(egresados_ids), desc="egresados"
    ):
        universidad, cod_universidad = alumnos_df.loc[
            alumnos_df["id"] == id_alumno, ["universidad", "cod_universidad"]
        ].values[0]
        cod_plan = matricula_df.loc[matricula_df["id"] == id_alumno, "cod_plan"].values[
            0
        ]
        ultimo_curso_aca = matricula_df.loc[
            matricula_df["id"] == id_alumno, "curso_aca"
        ].max()
...
asignaturas_matriculadas_df = generate_asignaturas_matriculadas(
    matricula_df, cod_asignaturas_dict
)
lineas_actas_df = generate_lineas_actas(asignaturas_matriculadas_df)
docentes_df = generate_docentes(
    num_docentes, universidades_dict, cod_asignaturas_dict, asignaturas_dict
)
ebau_prueba_df = generate_ebau_prueba(
    num_alumnos, alumnos_df["id"].tolist(), universidades_dict
)
